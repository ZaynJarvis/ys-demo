...
export default function NavBar() {
    ...
    useEffect(() => {
      const handleResize = () => {
        // Assuming 768px as a breakpoint for medium devices
        if (window.innerWidth >= 768) {
          setIsOpen(true);
        }
      };
      ...
      <nav className="fixed top-0 right-0 mt-4 mr-4 z-50">
        ...
        <li>
          ...
          <li>
            <a href="#gallary" onClick={() => setIsOpen(false)} className="text-white hover:text-gray-300 cursor-pointer block p-4">Gallary</a>
          </li>
        </ul>
      </nav>
    );
